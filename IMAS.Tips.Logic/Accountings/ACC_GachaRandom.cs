...
namespace IMAS.Tips.Logic.Accountings
{
    public class ACC_GachaRandom
    {
        ...
        /// <returns></returns>
        public static List<T> GetRandomList<T>(List<T> list, int count) where T : Model_PlayerControllCharacter
        {
            ...
            {
                totalWeights += list[i].GachaWeight + 1;
            }
            ...
            {
                // （权重+1） + 从0到（总权重-1）的随机数
                var w = (list[i].GachaWeight + 1) + ran.Next(0, totalWeights);
